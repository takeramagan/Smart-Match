...
export function MatchJob ({ job, onClick }) {
  ...
  const logo = job_logo ? job_logo : 'defaultlogo.svg'
  const rating = job_rating ? job_rating : 'Not Disclosed'
  //add popover
  ...
  return (
    ...
    >
      ...
      <Box p={1} width={370}>
        <Typography>Company: {job_company}</Typography>
        <Typography>Rating: {rating}</Typography>
        <Typography>Location: {job_location}</Typography>
        <Typography>Type: {job_type.join(' ')}</Typography>
        <Typography>Summary:</Typography>
        <Typography>{job_summary}</Typography>
